export function fixedCharCodeAt(str: string, idx?: number) {
...
export function char2Unicode(char: string) {
  return charCode2Unicode(fixedCharCodeAt(char))
}
...
export function convertCodePoints(str: string) {
  return Array.from(str).map((char) => {
    return char.codePointAt(0)?.toString(16);
  });
}

export function gwname2char(gwname: string) {
  let results: string[] = []
  const names = gwname.split('-')
  for (const name of names) {
    const hanzi = ucode2char(name)
    results.push(hanzi)
  }
  return results.join("")
}

export function gwname2charWithoutIDC(gwname: string) {
  let results: string[] = []
  const names = gwname.split('-')
  for (const name of names) {
    const hanzi = ucode2char(name)
    if (isIDC(hanzi)) continue
    results.push(hanzi)
  }
  return results.join("")
}

export function ucode2char(code: string){
  //"u4e00" -> "一"
  return String.fromCodePoint(Number('0x' + code.substring(1, code.length)))
}

export function isIDC(part: string) {
